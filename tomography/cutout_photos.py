...
photo_fn_format = os.path.join(photo_dir, "plak_%03d_%s%d.JPG")
    ...
    for plak in range(30):
        for kant in ['a', 'v']:
            for poging in range(9):
                ...
                fname = photo_fn_format % photo_id
...
def generate_cutout(ph_id, th, closing_size, opening_size, th_below=True):
    orig_img = io.imread(photo_fn_format % ph_id)[::-1, :, :]
    ...
    io.imsave(cutout_fn_format % ph_id, cutout_image[::-1,:,:])
